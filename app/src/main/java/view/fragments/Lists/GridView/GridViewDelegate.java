...
import view.glide.GlideManager;
import view.fragments.IAdapterDelegates;
...
public class GridViewDelegate implements IAdapterDelegates {
    ...
    public GridViewDelegate(Context mContext,  boolean allUnlocked) {
        this.mContext = mContext;
        this.allUnlocked = allUnlocked ;
    }
    ...
    public void onBindViewHolder(ArrayList itemsList, RecyclerView.ViewHolder holder, int position) {
        ...
        if(   isLocked(listItem)   )
            mHolder.toggleLock(View.VISIBLE);
    }
    ...
    private void bindImage(Object listItem , GridViewHolder mHolder){
        ...
        if (    hasImage(listItem)  ) {
            ...
            if(glideManager != null)
                glideManager.setGlideImage(image ,  mHolder.thumbnail );
            else
                mHolder.thumbnail.setImageResource( (int)  image   );
        }else {
    ...
    public class GridViewHolder extends RecyclerView.ViewHolder{
        ...
        public GridViewHolder(View itemView   ) {
            ...
            if( glideManager != null)
                calculateThumbnailDimens();

         }

         private void calculateThumbnailDimens(){
